...
```sh
make docker-build docker-push IMG=<some-registry>/hbase-k8s-operator:tag
```
...
```sh
make deploy IMG=<some-registry>/hbase-k8s-operator:tag
```
...
```sh
make build-installer IMG=<some-registry>/hbase-k8s-operator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/hbase-k8s-operator/<tag or branch>/dist/install.yaml
```
